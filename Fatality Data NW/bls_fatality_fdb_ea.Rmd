...
naics <- naics %>% group_by(code2) %>% fill(level_2)

# Add notation for Primary (1°) source and Secondary (2°) source
df <- df %>% 
  mutate(across(num_range("Char_",2:5), 
                ~ ifelse(!is.na(.x) & Char_1 == "Primary source", 
                         paste(.x, "(1°)"), .x))) %>% 
  mutate(across(num_range("Char_",2:5), 
                ~ ifelse(!is.na(.x) & Char_1 == "Secondary source", 
                         paste(.x, "(2°)"), .x)))

# Create a data frame of characteristic values/levels for selection list
charvals = df %>% 
...
renderPlotly({
  ...
  plotdata = NULL
  try({
    plotdata = (total_fatalities())
  }, silent = T)
  
  # Check for required values
  validate(
    need(input$naicsval != "", 
         paste("Please use the menu to select an industry group.",
               "Use the > symbol to navigate through levels.", sep = "\n")),
    need(input$char != "", 
         paste("Please use the menu to choose injury characteristics.", 
               "Use the > symbol to navigate through levels.", sep = "\n"))
  )
    ...
    p1 = ggplot() + 
      annotate("text", x = 1, y = 1, size = 4, 
        label = paste(
          ...
          "Please choose injury characteristics.", 
          "Use the > symbol to navigate through levels.", sep = "\n")
      ) + theme_void()
