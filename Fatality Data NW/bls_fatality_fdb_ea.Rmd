...
pacman::p_load(here, readr, dplyr, tidyr, stringr, purrr, forcats,DT,data.table,
...
charvals = df %>% select(Char_1, Char_2, Char_3, Char_4, Char_5) %>% distinct()
#charvals$Char = "Total"
...
treecheckInput(
    ...
    choices = make_tree(
      data = naics, levels = c("level_2", "level_3", "level_4", "level_5","level_6") ))
    
sliderInput("year", "Year:",
            min = min(df$Year, na.rm=T), sep = "",
            max = max(df$Year, na.rm=T), value = c(min(df$Year, na.rm=T), max(df$Year, na.rm=T)))
...
treecheckInput(
    ...
    choices = make_tree(
     data = charvals, levels = c( "Char_1", "Char_2", "Char_3") ))
...
renderPlotly({
  ...
  if(nrow(plotdata) >1 & length(input$naicsval) > 0 & length(input$char) > 0){
