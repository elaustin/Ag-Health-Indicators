...
total_fatalities <- reactive ({
  ...
  mycode = naics %>% 
    filter(name  == input$naicsval) %>%
    select(code)
  mycode = mycode$code
    
  df_total <- df %>% 
  filter( State %in% input$state, code %in% mycode) %>%
  group_by(Year, State) %>% 
...
renderPlot({
  ...
  if(!is.null(nrow(plotdata))){
 ggplot(plotdata, aes(x = Year, y = Fatalities)) + geom_point() + 
    facet_wrap("State", nrow = 1) + geom_smooth()
  }
