...
df <- df %>% 
  mutate(Char_2 = ifelse(!is.na(Char_2) & Char_1 == "Primary source", 
                         paste(Char_2, "(1°)"), Char_2),
         Char_3 = ifelse(!is.na(Char_3) & Char_1 == "Primary source", 
                         paste(Char_3, "(1°)"), Char_3),
         Char_4 = ifelse(!is.na(Char_4) & Char_1 == "Primary source", 
                         paste(Char_4, "(1°)"), Char_4),
         Char_5 = ifelse(!is.na(Char_5) & Char_1 == "Primary source", 
                         paste(Char_5, "(1°)"), Char_5),
         Char_2 = ifelse(!is.na(Char_2) & Char_1 == "Secondary source", 
                         paste(Char_2, "(2°)"), Char_2),
         Char_3 = ifelse(!is.na(Char_3) & Char_1 == "Secondary source", 
                         paste(Char_3, "(2°)"), Char_3),
         Char_4 = ifelse(!is.na(Char_4) & Char_1 == "Secondary source", 
                         paste(Char_4, "(2°)"), Char_4),
         Char_5 = ifelse(!is.na(Char_5) & Char_1 == "Secondary source", 
                         paste(Char_5, "(2°)"), Char_5))
...
total_fatalities <- reactive ({
  ...
  mycode = naics %>% filter(name %in% input$naicsval) %>% select(code)
  mycode = mycode$code
  ...
  df_total <- mydata %>%
    ...
    #mutate(across(starts_with("Char_"), ~ str_remove(.x, " \\([12]°\\)"))) %>%
  ...
  df_total = df_total %>% drop_na(Fatalities)
  df_total
  })
  ...
  if (nrow(plotdata) > 0 &
    ...
    p <- ggplot(plotdata, aes(x = Year, y = Fatalities,
      ...
      scale_y_continuous(breaks = integer_breaks(),
                         limits = c(0, max(plotdata$Fatalities, na.rm = T) +
                                      5)) +
    ...
    p1 = ggplot() +                      # Draw ggplot2 plot with text only
      annotate("text", x = 1, y = 1, size = 4, 
        label = paste(
          "Please use the menu to select an industy group.", 
          "Use the > symbol to navigate through levels.", sep = "\n")
      ) + theme_void()
    ...
    p2 = ggplot() + 
      annotate("text", x = 1, y = 1, size = 2,
        label = paste(
          "No data exists for the combination of variables you selected.", 
          "Please modify your selection.", sep = "\n")
    ) + theme_void()
